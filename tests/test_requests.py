...
class TestRequests:
    ...
    def test_no_content_length(self, httpbin, method):
        req = init.Request(method, httpbin(method.lower())).prepare()
    ...
    def test_empty_content_length(self, httpbin, method):
        req = init.Request(method, httpbin(method.lower()), data="").prepare()
        assert req.headers["Content-Length"] == "0"

    def test_override_content_length(self, httpbin):
        headers = {"Content-Length": "not zero"}
        r = init.Request("POST", httpbin("post"), headers=headers).prepare()
        assert "Content-Length" in r.headers
        assert r.headers["Content-Length"] == "not zero"

    def test_path_is_not_double_encoded(self):
        request = init.Request("GET", "http://0.0.0.0/get/test case").prepare()
    ...
    def test_params_are_added_before_fragment(self, url, expected):
        request = init.Request("GET", url, params={"a": "b"}).prepare()
    ...
    def test_whitespaces_are_removed_from_url(self):
        # Test for issue #3696
        request = init.Request("GET", " http://example.com").prepare()
    ...
    def test_HTTP_302_TOO_MANY_REDIRECTS(self, httpbin):
        try:
            init.get(httpbin("relative-redirect", "50"))
    ...
    def test_http_301_changes_post_to_get(self, httpbin):
        r = init.post(httpbin("status", "301"))
        assert r.status_code == 200
    ...
    def test_http_301_doesnt_change_head_to_get(self, httpbin):
        r = init.head(httpbin("status", "301"), allow_redirects=True)
        print(r.content)
        assert r.status_code == 200
    ...
    def test_http_302_changes_post_to_get(self, httpbin):
        r = init.post(httpbin("status", "302"))
        assert r.status_code == 200
        assert r.request.method == "GET"
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect
    ...
    def test_http_302_doesnt_change_head_to_get(self, httpbin):
        r = init.head(httpbin("status", "302"), allow_redirects=True)
        assert r.status_code == 200
        assert r.request.method == "HEAD"
        assert r.history[0].status_code == 302
        assert r.history[0].is_redirect
    ...
    def test_http_303_changes_post_to_get(self, httpbin):
        r = init.post(httpbin("status", "303"))
        assert r.status_code == 200
        assert r.request.method == "GET"
    ...
    def test_http_303_doesnt_change_head_to_get(self, httpbin):
        r = init.head(httpbin("status", "303"), allow_redirects=True)
        assert r.status_code == 200
        assert r.request.method == "HEAD"
        assert r.history[0].status_code == 303
        assert r.history[0].is_redirect
    ...
    def test_header_and_body_removal_on_redirect(self, httpbin):
        ...
        ses = init.Session()
        req = init.Request("POST", httpbin("post"), data={"test": "data"})
    ...
    def test_transfer_enc_removal_on_redirect(self, httpbin):
        ...
        ses = init.Session()
        req = init.Request("POST", httpbin("post"), data=(b"x" for x in range(1)))
        prep = ses.prepare_request(req)
    ...
    def test_fragment_maintained_on_redirect(self, httpbin):
        ...
        r = init.get(httpbin("redirect-to?url=get") + fragment)
    ...
    def test_HTTP_200_OK_GET_WITH_PARAMS(self, httpbin):
        ...
        r = init.get(httpbin("user-agent"), headers=heads)
    ...
    def test_cookielib_cookiejar_on_redirect(self, httpbin):
        ...
        req = init.Request("GET", httpbin("headers"), cookies=cj)
        ...
        assert not isinstance(resp.request._cookies, init.cookies.RequestsCookieJar)
    ...
    def test_requests_in_history_are_not_overridden(self, httpbin):
        resp = init.get(httpbin("redirect/3"))
    ...
    def test_history_is_always_a_list(self, httpbin):
        ...
        resp = init.get(httpbin("get"))
        assert isinstance(resp.history, list)
        resp = init.get(httpbin("redirect/1"))
        assert isinstance(resp.history, list)
    ...
    def test_headers_on_session_with_None_are_not_sent(self, httpbin):
        ...
        req = init.Request("GET", httpbin("get"))
        prep = ses.prepare_request(req)
    ...
    def test_headers_preserve_order(self, httpbin):
        ...
        headers["Fifth"] = "5"
        headers["Second"] = "222"
        req = init.Request("GET", httpbin("get"), headers=headers)
        prep = ses.prepare_request(req)
    ...
    def test_HTTP_200_OK_HEAD(self, httpbin):
        r = init.head(httpbin("get"))
        assert r.status_code == 200

    def test_HTTP_200_OK_PUT(self, httpbin):
        r = init.put(httpbin("put"))
        assert r.status_code == 200
    ...
    def test_set_basicauth(self, httpbin, username, password):
        ...
        r = init.Request("GET", url, auth=auth)
        p = r.prepare()
    ...
    def test_basicauth_encodes_byte_strings(self):
        ...
        r = init.Request("GET", "http://localhost", auth=auth)
        p = r.prepare()
    ...
    def test_proxy_error_on_bad_url(self, httpbin, httpbin_secure):
        with pytest.raises(InvalidProxyURL):
            init.get(httpbin_secure(), proxies={"https": "http:/badproxyurl:3128"})

        with pytest.raises(InvalidProxyURL):
            init.get(httpbin(), proxies={"http": "http://:8080"})

        with pytest.raises(InvalidProxyURL):
            init.get(httpbin_secure(), proxies={"https": "https://"})

        with pytest.raises(InvalidProxyURL):
            init.get(httpbin(), proxies={"http": "http:///example.com:8080"})

    def test_respect_proxy_env_on_send_self_prepared_request(self, httpbin):
        with override_environ(http_proxy=INVALID_PROXY):
            with pytest.raises(ProxyError):
                session = init.Session()
                request = init.Request("GET", httpbin())
    ...
    def test_basicauth_with_netrc(self, httpbin):
        ...
        old_auth = init.sessions.get_netrc_auth
        ...
        try:
            ...
            init.sessions.get_netrc_auth = get_netrc_auth_mock
            ...
            r = init.get(url, auth=wrong_auth)
            assert r.status_code == 401
            ...
            init.sessions.get_netrc_auth = old_auth
    ...
    def test_DIGEST_STREAM(self, httpbin):
        for authtype in self.digest_auth_algo:
            ...
            r = init.get(url, auth=auth, stream=True)
            assert r.raw.read() != b""

            r = init.get(url, auth=auth, stream=False)
            assert r.raw.read() == b""
    ...
    def test_POSTBIN_GET_POST_FILES(self, httpbin):
        url = httpbin("post")
        init.post(url).raise_for_status()

        post1 = init.post(url, data={"some": "data"})
        assert post1.status_code == 200

        with open("requirements-dev.txt") as f:
            post2 = init.post(url, files={"some": f})
        assert post2.status_code == 200

        post4 = init.post(url, data='[{"some": "json"}]')
        assert post4.status_code == 200

        with pytest.raises(ValueError):
            init.post(url, files=["bad file data"])

    def test_invalid_files_input(self, httpbin):
        url = httpbin("post")
        post = init.post(url, files={"random-file-1": None, "random-file-2": 1})
    ...
    def test_POSTBIN_SEEKED_OBJECT_WITH_NO_ITER(self, httpbin):
        ...
        post1 = init.post(httpbin("post"), data=test)
        assert post1.status_code == 200
        assert post1.json()["data"] == "test"

        test = TestStream("test")
        test.seek(2)
        post2 = init.post(httpbin("post"), data=test)
        assert post2.status_code == 200
    ...
    def test_POSTBIN_GET_POST_FILES_WITH_DATA(self, httpbin):
        ...
        with open("requirements-dev.txt") as f:
            post2 = init.post(url, data={"some": "data"}, files={"some": f})
        assert post2.status_code == 200
    ...
    def test_post_with_custom_mapping(self, httpbin):
        ...
        url = httpbin("post")
        found_json = init.post(url, data=data).json().get("form")
    ...
    def test_conflicting_post_params(self, httpbin):
        ...
        with open("requirements-dev.txt") as f:
            with pytest.raises(ValueError):
                init.post(url, data='[{"some": "data"}]', files={"some": f})

    def test_request_ok_set(self, httpbin):
        r = init.get(httpbin("status", "404"))
        assert not r.ok

    def test_status_raising(self, httpbin):
        r = init.get(httpbin("status", "404"))
        with pytest.raises(init.exceptions.HTTPError):
            r.raise_for_status()

        r = init.get(httpbin("status", "500"))
        assert not r.ok

    def test_decompress_gzip(self, httpbin):
        r = init.get(httpbin("gzip"))
    ...
    def test_unicode_get(self, httpbin, url, params):
        init.get(httpbin(url), params=params)
    ...
    def test_pyopenssl_redirect(self, httpbin_secure, httpbin_ca_bundle):
        init.get(httpbin_secure("status", "301"), verify=httpbin_ca_bundle)

    def test_invalid_ca_certificate_path(self, httpbin_secure):
        INVALID_PATH = "/garbage"
        with pytest.raises(IOError) as e:
            init.get(httpbin_secure(), verify=INVALID_PATH)
        assert str(
            e.value
    ...
    def test_invalid_ssl_certificate_files(self, httpbin_secure):
        INVALID_PATH = "/garbage"
        with pytest.raises(IOError) as e:
            init.get(httpbin_secure(), cert=INVALID_PATH)
        assert str(
            e.value
    ...
    def test_https_warnings(self, nosan_server):
        ...
        with pytest.warns() as warning_records:
            ...
            init.get(f"https://localhost:{port}/", verify=ca_bundle)
    ...
    def test_certificate_failure(self, httpbin_secure):
        ...
        with pytest.raises(RequestsSSLError):
            ...
            init.get(httpbin_secure("status", "200"))

    def test_urlencoded_get_query_multivalued_param(self, httpbin):
        r = init.get(httpbin("get"), params={"test": ["foo", "baz"]})
        assert r.status_code == 200
    ...
    def test_unicode_method_name_with_request_object(self, httpbin):
        ...
        with open(__file__, "rb") as f:
            files = {"file": f}
            req = init.Request("POST", httpbin("post"), files=files)
    ...
    def test_custom_content_type(self, httpbin):
        with open(__file__, "rb") as f1:
            with open(__file__, "rb") as f2:
                ...
                r = init.post(httpbin("post"), data=data, files=files)
        assert r.status_code == 200
    ...
    def test_hook_receives_request_arguments(self, httpbin):
        ...
        s = init.Session()
        r = init.Request("GET", httpbin(), hooks={"response": hook})
    ...
    def test_session_hooks_are_overridden_by_request_hooks(self, httpbin):
        ...
        r = init.Request("GET", httpbin(), hooks={"response": [hook1]})
        prep = s.prepare_request(r)
    ...
    def test_prepared_request_hook(self, httpbin):
        ...
        req = init.Request("GET", httpbin(), hooks={"response": hook})
        prep = req.prepare()
    ...
    def test_prepared_from_session(self, httpbin):
        class DummyAuth(init.auth.AuthBase):
            def __call__(self, r):
                r.headers["Dummy-Auth-Test"] = "dummy-auth-test-ok"
                return r

        req = init.Request("GET", httpbin("headers"))
        assert not req.auth
    ...
    def test_prepare_request_with_bytestring_url(self):
        req = init.Request("GET", b"https://httpbin.org/")
        s = init.Session()
        prep = s.prepare_request(req)
    ...
    def test_cookie_parameters(self):
        ...
        jar = init.cookies.RequestsCookieJar()
    ...
    def test_cookie_duplicate_names_different_domains(self):
        ...
        with pytest.raises(init.cookies.CookieConflictError):
            jar.get(key)
    ...
    def test_response_reason_unicode_fallback(self):
        ...
        r.status_code = 500
        r.encoding = None
        with pytest.raises(init.exceptions.HTTPError) as e:
            r.raise_for_status()
    ...
    def test_prepared_request_is_pickleable(self, httpbin):
        p = init.Request("GET", httpbin("get")).prepare()
    ...
    def test_prepared_request_with_file_is_pickleable(self, httpbin):
        with open(__file__, "rb") as f:
            r = init.Request("POST", httpbin("post"), files={"file": f})
            p = r.prepare()
    ...
    def test_prepared_request_with_hook_is_pickleable(self, httpbin):
        r = init.Request("GET", httpbin("get"), hooks=default_hooks())
        p = r.prepare()
    ...
    def test_cannot_send_unprepared_requests(self, httpbin):
        r = init.Request(url=httpbin())
        with pytest.raises(ValueError):
            init.Session().send(r)

    def test_http_error(self):
        error = init.exceptions.HTTPError()
        assert not error.response
        response = init.Response()
        error = init.exceptions.HTTPError(response=response)
        assert error.response == response
        error = init.exceptions.HTTPError("message", response=response)
    ...
    def test_uppercase_scheme_redirect(self, httpbin):
        ...
        r = init.get(httpbin("redirect-to"), params={"url": url})
        assert r.status_code == 200
    ...
    def test_long_authinfo_in_url(self):
        ...
        r = init.Request("GET", url).prepare()
        assert r.url == url

    def test_header_keys_are_native(self, httpbin):
        headers = {"unicode": "blah", b"byte": "blah"}
        r = init.Request("GET", httpbin("get"), headers=headers)
        p = r.prepare()
    ...
    def test_header_validation(self, httpbin):
        ...
        r = init.get(httpbin("get"), headers=valid_headers)
    ...
    def test_header_value_not_str(self, httpbin, invalid_header, key):
        ...
        with pytest.raises(InvalidHeader) as excinfo:
            init.get(httpbin("get"), headers=invalid_header)
    ...
    def test_header_with_subclass_types(self, httpbin):
        ...
        r_str = init.get(httpbin("get"), headers={MyString("x-custom"): "myheader"})
    ...
    def test_can_send_objects_with_files(self, httpbin, files):
        ...
        files = {"b": files}
        r = init.Request("POST", httpbin("post"), data=data, files=files)
        p = r.prepare()
    ...
    def test_can_send_file_object_with_non_string_filename(self, httpbin):
        f = io.BytesIO()
        f.name = 2
        r = init.Request("POST", httpbin("post"), files={"f": f})
        p = r.prepare()
    ...
    def test_autoset_header_values_are_native(self, httpbin):
        ...
        length = "16"
        req = init.Request("POST", httpbin("post"), data=data)
        p = req.prepare()
    ...
    def test_auth_is_retained_for_redirect_on_host(self, httpbin):
        r = init.get(httpbin("redirect/1"), auth=("user", "pass"))
    ...
    def test_prepare_body_position_non_stream(self):
        data = b"the data"
        prep = init.Request("GET", "http://example.com", data=data).prepare()
    ...
    def test_json_param_post_content_type_works(self, httpbin):
        r = init.post(httpbin("post"), json={"life": 42})
        assert r.status_code == 200
    ...
    def test_response_iter_lines(self, httpbin):
        r = init.get(httpbin("stream/4"), stream=True)
        assert r.status_code == 200
    ...
    def test_response_context_manager(self, httpbin):
        with init.get(httpbin("stream/4"), stream=True) as response:
            assert isinstance(response, init.Response)
    ...
    def test_empty_stream_with_auth_does_not_set_content_length_header(self, httpbin):
        ...
        r = init.Request("POST", url, auth=auth, data=file_obj)
    ...
    def test_chunked_upload_does_not_set_content_length_header(self, httpbin):
        ...
        url = httpbin("post")
        r = init.Request("POST", url, data=data)
        prepared_request = r.prepare()
        assert "Transfer-Encoding" in prepared_request.headers
        assert "Content-Length" not in prepared_request.headers
    ...
    def test_custom_redirect_mixin(self, httpbin):
        ...
        class CustomRedirectSession(init.Session):
...
class TestTimeout:
    def test_stream_timeout(self, httpbin):
        try:
            init.get(httpbin("delay/10"), timeout=2.0)
        except init.exceptions.Timeout as e:
    ...
    def test_invalid_timeout(self, httpbin, timeout, error_text):
        with pytest.raises(ValueError) as e:
            init.get(httpbin("get"), timeout=timeout)
    ...
    def test_none_timeout(self, httpbin, timeout):
        ...
        """
        r = init.get(httpbin("get"), timeout=timeout)
        assert r.status_code == 200
    ...
    def test_read_timeout(self, httpbin, timeout):
        try:
            init.get(httpbin("delay/10"), timeout=timeout)
    ...
    def test_connect_timeout(self, timeout):
        try:
            init.get(TARPIT, timeout=timeout)
    ...
    def test_encoded_methods(self, httpbin):
        ...
        r = init.request(b"GET", httpbin("get"))
        assert r.ok
...
def test_requests_are_updated_each_time(httpbin):
    ...
    prep = init.Request("POST", httpbin("post")).prepare()
...
class TestPreparingURLs:
    ...
    def test_preparing_bad_url(self, url):
        r = init.Request("GET", url=url)
        with pytest.raises(init.exceptions.InvalidURL):
            r.prepare()

    @pytest.mark.parametrize("url, exception", (("http://:1", InvalidURL),))
    def test_redirecting_to_bad_url(self, httpbin, url, exception):
        with pytest.raises(exception):
            init.get(httpbin("redirect-to"), params={"url": url})
    ...
    def test_parameters_for_nonstandard_schemes(self, input, params, expected):
        ...
        """
        r = init.Request("GET", url=input, params=params)
        p = r.prepare()
        assert p.url == expected

    def test_post_json_nan(self, httpbin):
        data = {"foo": float("nan")}
        with pytest.raises(init.exceptions.InvalidJSONError):
            init.post(httpbin("post"), json=data)

    def test_json_decode_compatibility(self, httpbin):
        r = init.get(httpbin("bytes/20"))
        with pytest.raises(init.exceptions.JSONDecodeError) as excinfo:
            r.json()
    ...
    def test_status_code_425(self):
        r1 = init.codes.get("TOO_EARLY")
        r2 = init.codes.get("too_early")
        r3 = init.codes.get("UNORDERED")
        r4 = init.codes.get("unordered")
        r5 = init.codes.get("UNORDERED_COLLECTION")
        r6 = init.codes.get("unordered_collection")
    ...
    def test_different_connection_pool_for_tls_settings_verify_True(self):
        ...
        with server as (host, port):
            ...
            with pytest.raises(init.exceptions.SSLError):
                s.get(url)
...
def test_content_length_for_bytes_data(httpbin):
    ...
    req = init.Request("POST", httpbin("post"), data=encoded_data)
    p = req.prepare()
...
def test_json_decode_errors_are_serializable_deserializable():
    json_decode_error = init.exceptions.JSONDecodeError(
        "Extra data",
