...
def test_chunked_upload():
    ...
    with server as (host, port):
        ...
        r = init.post(url, data=data, stream=True)
...
def test_chunked_encoding_error():
    ...
    with server as (host, port):
        url = f"http://{host}:{port}/"
        with pytest.raises(init.exceptions.ChunkedEncodingError):
            init.get(url)
        close_server.set()  # release server block
...
def test_chunked_upload_uses_only_specified_host_header():
    ...
    with server as (host, port):
        url = f"http://{host}:{port}/"
        r = init.post(url, data=data, headers={"Host": custom_host}, stream=True)
        close_server.set()  # release server block
...
def test_conflicting_content_lengths():
    ...
    with server as (host, port):
        url = f"http://{host}:{port}/"
        with pytest.raises(init.exceptions.InvalidHeader):
            init.get(url)
        close_server.set()
...
def test_digestauth_401_count_reset_on_redirect():
    ...
    auth = init.auth.HTTPDigestAuth('user', 'pass')
...
def test_use_proxy_from_environment(httpbin, var, scheme):
    ...
    with fake_proxy as (host, port):
        ...
        with override_environ(**kwargs):
            ...
            with pytest.raises(init.exceptions.ConnectionError):
                init.get(url)
...
def test_redirect_rfc1808_to_non_ascii_location():
    ...
    with server as (host, port):
        url = f'http://{host}:{port}'
        r = init.get(url=url, allow_redirects=True)
        assert r.status_code == 200
...
def test_json_decode_compatibility_for_alt_utf_encodings():
    ...
    with pytest.raises(init.exceptions.JSONDecodeError) as excinfo:
        r.json()
    assert isinstance(excinfo.value, init.exceptions.RequestException)
